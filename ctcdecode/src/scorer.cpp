...
std::vector<std::string> Scorer::make_ngram(PathTrie* prefix) {
  ...
  for (int order = 0; order < max_order_; order++) {
    ...
    if (is_character_based_) {
      new_node = current_node->get_path_vec(prefix_vec, prefix_steps, char_probs, SPACE_ID_, 1);
      current_node = new_node;
    } else {
      new_node = current_node->get_path_vec(prefix_vec, prefix_steps, char_probs, SPACE_ID_);
